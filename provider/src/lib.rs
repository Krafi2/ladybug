...
impl Factory {
    ...
    fn create_transaction(
        ...
        match self.get_raw_provider(id.inner, ctx) {
            Ok(transactor) => transactor
                .new_transaction(args, packages, ctx)
...
impl Manager {
    ...
    pub fn new_transaction(
        ...
        let provider = match provider {
            ...
            None => {
                ctx.emit(TransactionError::Parse(eval::ParamError::NotFound {
                    span: args_span,
                    name: "provider",
    ...
    pub fn install(
        ...
        match self.server.super_ctx(ctx) {
            Ok(ctx) => {
                let (res, state) = provider.install(&transaction.payload, ctx);
                Some((
                    res,
                    State {
                        payload: state,
    ...
    pub fn remove(
        ...
        match self.server.super_ctx(ctx) {
            Ok(ctx) => provider.remove(&transaction.payload, state.map(|s| s.payload), ctx),
...
pub struct ExecutionCtx<'a> {
    set_msg: Box<dyn FnMut(String) + 'a>,
}

impl<'a> ExecutionCtx<'a> {
    pub fn new(set_msg: Box<dyn FnMut(String) + 'a>) -> Self {
        Self { set_msg }
    }
